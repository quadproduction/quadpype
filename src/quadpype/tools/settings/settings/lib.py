...
def create_deffered_value_change_timer(callback):
    ...
    The UI won't trigger all callbacks on each value change but after predefined
    time. The timer is reset on each start, so callback is triggered after
...
def create_add_btn(parent):
    ...
    add_btn.setFocusPolicy(QtCore.Qt.FocusPolicy.ClickFocus)
    return add_btn


def create_remove_btn(parent):
    remove_btn = SettingsToolBtn("remove", parent)
    remove_btn.setFocusPolicy(QtCore.Qt.FocusPolicy.ClickFocus)
    return remove_btn
