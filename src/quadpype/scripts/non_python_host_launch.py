...
def main(argv):
    ...
    host_name = os.environ["QUADPYPE_HOST_NAME"].lower()
    if host_name == "photoshop":
        # TODO refactor launch logic according to AE
        from quadpype.hosts.photoshop.api.lib import main
    elif host_name == "aftereffects":
        from quadpype.hosts.aftereffects.api.launch_logic import main
    elif host_name == "harmony":
        from quadpype.hosts.harmony.api.lib import main
    else:
        title = "Unknown host name"
        message = (
            "BUG: Environment variable QUADPYPE_HOST_NAME contains unknown"
            " host name \"{}\""
        ).format(host_name)
