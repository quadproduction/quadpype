...
def stampCreateAnchor(node = None, extra_tags = [], no_default_tag = False, autoClickedOk=False):
    '''
    ...
    if node is not None:
        ...
        window_title = "New Stamp: "+str(node.name())+str(autoClickedOk)
    else:
        default_title = "Stamp"
        default_tags = ""
        node_type = ""
    ...
    if autoClickedOk:
        anchor_title = new_anchor_panel.anchorTitle_edit.text().strip()
        anchor_tags = new_anchor_panel.anchorTags_edit.text().strip()
...
def goStamp(ns="", autoClickedOk=False):
    ...
    if not len(ns):
        if not totalAnchors(): # If no anchors on the script, create an anchor with no input
            stampCreateAnchor(no_default_tag = True, autoClickedOk=autoClickedOk)
        else:
        ...
        for n in ns:
            if n in NodeExceptionClasses:
                ...
                if n.knob("stamp_tags"):
                    stampCreateAnchor(n, extra_tags = n.knob("stamp_tags").value().split(","), no_default_tag = True, autoClickedOk=autoClickedOk)
                else:
                    extra_tags = stampCreateAnchor(n, extra_tags = extra_tags, autoClickedOk=autoClickedOk) # Create anchor via anchor creation panel
