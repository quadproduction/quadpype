...
$QUADPYPE_DB_URI_CONTAINER_PORT = 27017
...
if ($? -eq $true) {
    docker start $QUADPYPE_DB_URI_CONTAINER_NAME
    return 0
}

# Check if there is already a Docker container using the required port
docker ps -a | findstr $QUADPYPE_DB_URI_CONTAINER_PORT > $null
if ($? -eq $true) {
...
docker run -p "${QUADPYPE_DB_URI_CONTAINER_PORT}:${QUADPYPE_DB_URI_CONTAINER_PORT}" --name $QUADPYPE_DB_URI_CONTAINER_NAME -d mongo > $null
