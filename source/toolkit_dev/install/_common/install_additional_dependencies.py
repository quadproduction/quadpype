...
def install_qtbinding(
    ...
    if platform_name == "darwin":
        sqldrivers_dir = python_vendor_dir.joinpath(package, "Qt", "plugins", "sqldrivers")
...
def install_thirdparty(pyproject, platform_name, quadpype_root):
...
def main():
    ...
    while (repo_root.parts[-1]) != "source":
        repo_root = repo_root.parent
    repo_root.resolve()

    python_vendor_dir = repo_root.joinpath("vendor", "python")
    ...
    pyproject = toml.load(repo_root.joinpath("pyproject.toml"))
    ...
    install_thirdparty(pyproject, platform_name, repo_root)
