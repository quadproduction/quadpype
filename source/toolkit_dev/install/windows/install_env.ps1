...
$PATH_QUADPYPE_SOURCE_DIR = "$($PATH_QUADPYPE_DIR)\source"
...
if ($CURR_USER_PATH.IndexOf("$($PATH_QUADPYPE_SOURCE_DIR)$([IO.Path]::PathSeparator)") -Eq -1) {
    [System.Environment]::SetEnvironmentVariable("Path", "$($PATH_QUADPYPE_SOURCE_DIR)$([IO.Path]::PathSeparator)$($CURR_USER_PATH)", [System.EnvironmentVariableTarget]::User)
}
...
Set-Location -Path "$($PATH_QUADPYPE_DIR)\source"

# 5.B Check validity of the QuadPype version
$PATH_QUADPYPE_VERSION_FILE = "$($PATH_QUADPYPE_DIR)\source\quadpype\version.py"
...
function Install-Poetry() {
    ...
    $env:POETRY_HOME="$PATH_QUADPYPE_DIR\source\.poetry"
...
if (-not (Test-Path -PathType Leaf -Path "$($PATH_QUADPYPE_DIR)\source\poetry.lock")) {
...
################################################
& "$($PATH_QUADPYPE_DIR)\source\toolkit_dev\activate\windows\activate.ps1"
...
#######################################################
& "$env:POETRY_HOME\bin\poetry" run python "$(Split-Path -Path $SCRIPT_DIR -Parent)\_common\install_additional_dependencies.py"

# 9. Set back the current location to the current script folder
###############################################################
Set-Location -Path $PATH_ORIGINAL_LOCATION

# 10. Execute the activate script
#################################
. "$($PATH_QUADPYPE_DIR)\source\toolkit_dev\activate\windows\activate.ps1"
