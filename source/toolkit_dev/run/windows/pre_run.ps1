...
if (($QUADPYPE_MONGO -eq "")) {
    ...
    exit 1
} elseIf (!(Test-Path -Path "$PATH_QUADPYPE_DIR")) {
...
$PATH_ADDITIONAL_ENV_FILE = "$((get-item $SCRIPT_DIR).Parent.FullName)\_common\.env"
...
New-Item "$($PATH_ADDITIONAL_ENV_FILE)" -ItemType File -Value "QUADPYPE_MONGO=$QUADPYPE_MONGO$([Environment]::NewLine)QUADPYPE_DIR=$PATH_QUADPYPE_DIR" | Out-Null
...
if ($DEV) {
    $MONGO_START_SCRIPT_PATH = "$($PATH_QUADPYPE_DIR)\source\toolkit_dev\database\windows\start_mongo.ps1"
