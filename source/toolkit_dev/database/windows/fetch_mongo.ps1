...
function reset_db() {
    $RESET_DB_SCRIPT_PATH = Join-Path -Path (Split-Path $SCRIPT_DIR -Parent) -ChildPath "_common\erase_pipe_databases.js"
...
function disable_module($MODULE_NAME) {
    $DISABLE_MODULE_SCRIPT_PATH = Join-Path -Path (Split-Path $SCRIPT_DIR -Parent) -ChildPath "_common\disable_module.js"
...
function change_root_dir($ROOT_DIR) {
    ...
    $CHANGE_ROOT_DIR_SCRIPT_PATH = Join-Path -Path (Split-Path $SCRIPT_DIR -Parent) -ChildPath "_common\change_root_dir.js"
...
function main {
    ...
    $MONGO_DELETE_SCRIPT_PATH = Join-Path -Path $SCRIPT_DIR -ChildPath "delete_mongo.ps1"
    ...
    $MONGO_START_SCRIPT_PATH = Join-Path -Path $SCRIPT_DIR -ChildPath "start_mongo.ps1"
